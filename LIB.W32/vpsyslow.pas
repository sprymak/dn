...
//█      System interface layer (OS/2 and Win32)          █
//█      ─────────────────────────────────────────────────█
...
{Cat
   18/10/2001 (Win32) - изменил функции работы с Clipboard-ом,
   теперь вместо описателя данных cf_Text и перекодировок
   Ansi-Oem используется описатель cf_OemText

   19/10/2001 (Win32) - теперь используется собственный
   RecodeCyrillicNames, который должен быть проинициализирован
   в модуле DnIni

   09/12/2001 (Win32) - окончательно вычистил загрузку
   VPKBDW32.DLL; убрал вызовы InitialiseKeyboardHandler из
   всех мест, кроме InitialiseConsole, вызов которой происходит
   при инициализации модуля

   10/12/2001 (Win32) - раньше установка позиции курсора происходила
   в отдельной нитке, теперь в эту же нитку добавил ещё и изменение
...
{$IFDEF LINUX}  Linux;           {$UNDEF WIN32}  {$ENDIF}
...
type
  // This type *must* be in sync with System.TSharedMem
