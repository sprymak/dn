...
var
  hKernel: Handle;
  CreateToolhelp32Snapshot: function(dwFlags,th32ProcessID: LongInt): Handle stdcall;
  Process32First: function(hSnapshot: Handle; lppe: PProcessEntry32): Boolean stdcall;
  Process32Next: function (hSnapshot: Handle; lppe: PProcessEntry32): Boolean stdcall;
  hSnapshot: Handle;
...
begin
  ...
  if (@CreateToolhelp32Snapshot=nil) or (@Process32First=nil) or (@Process32Next=nil) then
    Exit;
...
var
  hNTDll: Handle;
  NtQuerySystemInformation: function(Nmb: Integer; Ptr: Pointer; Size1, Size2: Integer): LongInt stdcall;
...
begin
  ...
  if @NtQuerySystemInformation<>nil then
    begin
      ...
      repeat
        P:=New(PProcessItem, Init);
        WideCharToMultiByte(CP_OEMCP, 0, Pointer(PInfo^.pszProcessName), -1, @P^.Name[1], 255, nil, nil);
        for I:=1 to 255 do
          if P^.Name[I]=#0 then
            Break;
        P^.Name[0]:=Char(I);
...
function ProcessKill(Pid: LongInt): Longint;
begin
  if TerminateProcess(OpenProcess(1, True, Pid), 1) then Result := 0
    else Result := 1;
end;
