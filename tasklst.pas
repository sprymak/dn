...
begin
  X1 := ScreenWidth-4;
  Y1 := ScreenHeight-5;
  if Collection^.Count < (Y1 - 4) then Y1 := Collection^.Count + 4;
  if Y1 < 10 then Y1 := 10;
  ...
  Insert(New(PButton, Init(R, GetString(dlSwitchButton), cmOk, bfDefault)));
  R.Assign(X1-13, 5, X1-2, 7);
  if KillAllowed then
    begin
      Insert(New(PButton, Init(R, GetString(dlKillButton), cmButton1, bfNormal)));
...
{$IFDEF OS2} {JO: в Win32 пока не доделано}
  ...
  while (I < Collection^.Count) and
    (PProcessItem(Collection^.At(I))^.Pid < GetCurPid) do Inc(I);
  if I >= Collection^.Count then I := 0;
...
begin
  ...
  or (Event.What=evKeyDown) and (Event.KeyCode=kbDel) then
    EndModal(cmButton1);
  ...
  case Desktop^.ExecView(Dialog) of
    cmOk:
      if Dialog^.ListBox^.List^.Count > 0 then
        if ProcessSwitch(PProcessItem(Collection^.At(Dialog^.ListBox^.Focused))^.Pid) <> 0
          then
            begin
              Beep(100, 30);
              goto Ret;
    ...
    cmButton1:
      if (Dialog^.ListBox^.List^.Count > 0) and KillAllowed then
        if ProcessKill(PProcessItem(Collection^.At(Dialog^.ListBox^.Focused))^.Pid) <> 0
          then goto Ret;
  end;
  Dispose(Dialog, Done);
