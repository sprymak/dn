...
function TProcessList.GetText(Item: integer; MaxLen: integer):
    String;
  begin
...
Constructor TProcessDialog.Init(Collection: PProcessCollection);
  ...
  begin
    ...
    if KillAllowed then
      begin
        Insert(New(PButton, Init(R, GetString(dlKillButton),
          cmButton1, bfNormal)));
    ...
    case Desktop^.ExecView(Dialog) of
      cmOK:
        if Dialog^.ListBox^.List^.Count > 0 then
          if ProcessSwitch(PProcessItem(Collection^.At(Dialog^.
              ListBox^.Focused))^.Pid) <> 0
          then
            begin
              Beep(100, 30);
              goto ret;
      ...
      cmButton1:
        if (Dialog^.ListBox^.List^.Count > 0) and KillAllowed then
          if ProcessKill(PProcessItem(Collection^.At(Dialog^.ListBox^.
              Focused))^.Pid) <> 0
          then
            goto ret;
    end {case};
