...
function WinOpenObjectSh(hObject: lHandle; ulView: ULong; Flag:
    boolean): boolean;
  begin
    ...
    if WinOpenClipBrd(Anchor) then
      begin
        ...
        if P <> nil then
          begin
            l := StrLen(P)+1;
            if (DosQueryMem(P, l, Flags) = 0) and (Flags and 1 <> 0)
            then
              begin
                if DosAllocMem(Result, l, pag_Write or pag_Commit) = 0
                then
                  begin
                    Size := l;
                    Move(P^, Result^, Size);
...
function WinQueryTaskTitleSh(PSessId: longInt; var Title: String):
    longInt;
  var
    PTitle: PChar;
            ...
            begin
              ...
              if Anchor <> 0 then
                begin
                  ...
                  hwndDeskTop := WinQueryDesktopWindow(Anchor,
                    NULLHANDLE);
                end;
              ...
              if hSw <> 0 then
                begin
                  ...
                  if (PMStatus = 3) and (hwndDeskTop <> 0) then
                    MyWindow := WinQueryWindow(SwData.HWND,
                      QW_BOTTOM);
                  {SwData.hwnd - íâ® å¥­¤« à ¬ª¨,   ¢­ãâà¥­­®áâì
                       ï¢«ï¥âáï ¢ z-¯®àï¤ª¥ ¯®á«¥¤­¨¬ ®ª­®¬ ¢ à ¬ª¥}
                end;
            end
        end
      else
