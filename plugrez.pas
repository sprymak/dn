...
begin
  ...
  GetMem(P, SizeOf(TPluginRezData)-SizeOf(LongInt)+(SC+OC)*SizeOf(LongInt));
  with P^ do
    begin
      Stream := S;
      StringsCount := SC;
      ...
      if S^.Status <> stOk then
        begin
          Dispose(S, Done);
          FreeMem(P{, SizeOf(TPluginRezData)-SizeOf(LongInt)+(SC+OC)*SizeOf(LongInt)});
          OpenRez := 0;
          Exit;
        end;
    end;
...
begin
  if RezId <> 0 then
    with PPluginRezData(RezId)^ do
      begin
        Dispose(Stream, Done);
        FreeMem(Pointer(RezId){, SizeOf(TPluginRezData)-SizeOf(LongInt)+(StringsCount+ObjectsCount)*SizeOf(LongInt)})
      end;
end;
