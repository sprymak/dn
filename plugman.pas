  ...
  with DNFunctions^, SomeObjects2^, SomeObjects3^ do
    begin
      ...
      for I:=arcFirst to arcLast do
        if (ArchiveViewers[I]<>nil) and (ArchiveViewers[I]<>ArchiveViewers[I-1]) then
          Inc(W);
      Stream^.Write(W, SizeOf(W));
      for I:=arcFirst to arcLast do
        if (ArchiveViewers[I]<>nil) and (ArchiveViewers[I]<>ArchiveViewers[I-1]) then
...
begin
  ...
  with DNFunctions^, SomeObjects1^ do
    begin
      ...
      RestResourceSize:=PStream(ResourceStream)^.GetSize-NextResourceOffset;
      ...
      repeat
        ...
        PStream(ResourceStream)^.DoOpen(Open_Access_ReadWrite or Open_Share_DenyWrite);
          ...
          with P^ do
            begin
              ...
              or (DosQueryProcAddr(LibHandle, 0, 'DetectCreateArchiveObject', @DetectCreateArchiveObject)<>0) then
                begin
                  MessageBox(GetString(Integer(dlCantLoad))+PluginPath, nil, mfError+mfOKButton);
                  Dispose(P);
                  Result:=False;
                  Exit;
                end;
...
begin
  with DNFunctions^, SomeObjects2^, SomeObjects3^ do
    begin
      if ((Item^.PluginType and type_EventCatcher)<>0) and (Item^.Installed>=1) and (Item^.Installed<=EventCatchersCount) then
        begin
      ...
      if (Item^.PluginType and type_ArchiveViewer)<>0 then
        begin
          ...
          for I:=J+1 to J+ArchiveViewers[Item^.Installed]^.FormatsCount-1 do
...
constructor TPluginListItem.Init(PT: LongInt; FCC, LCC, {FLI, LLI, FDI, LDI, FHI, LHI,} FOT, LOT: Word; RFP: Boolean; PP: String);
var
  CommandCount: Word;
begin
...
begin
  with DNFunctions^ do
    begin
      ...
      R.Assign(5,17,22,19);
      InstallButton:=New(PButton, Init(R, GetString(Integer(dlPlugins9)), cmOk, bfDefault));
      Insert(InstallButton);
      R.Assign(24,17,41,19);
      Insert(New(PButton, Init(R, GetString(Integer(dlCloseButton)), cmCancel, bfNormal)));
      R.Assign(55,1,56,16);
  ...
  if Event.What=evCommand then
    case Event.Command of
          ...
          with PluginListItem^, DNFunctions^, SomeObjects1^ do
            begin
              ...
              if Installed<>0 then
                ...
                and ChangeMenuResource(UnInstallMenu, PluginListItem, PMenuView(LoadResource(Integer(dlgEditorMenu))), Integer(dlgEditorMenu), 1)
                and ChangeMenuResource(UnInstallMenu, PluginListItem, PMenuView(LoadResource(Integer(dlgWkzMenuBar))), Integer(dlgWkzMenuBar), 2) then
                  ListBox^.DrawView
                else
                  MessageBox(GetString(Integer(dlPlugins3)), @P, mfError+mfOkButton)
              else
                if Install(PluginListItem)
                and ChangeMenuResource(InstallMenu, PluginListItem, MenuBar, Integer(dlgMainMenu), 0)
                and ChangeMenuResource(InstallMenu, PluginListItem, PMenuView(LoadResource(Integer(dlgEditorMenu))), Integer(dlgEditorMenu), 1)
                and ChangeMenuResource(InstallMenu, PluginListItem, PMenuView(LoadResource(Integer(dlgWkzMenuBar))), Integer(dlgWkzMenuBar), 2) then
                  ListBox^.DrawView
                else
                  MessageBox(GetString(Integer(dlPlugins2)), @P, mfError+mfOkButton);
      ...
      cmCancel:
        Close;
      cmGetName:
        PString(Event.InfoPtr)^ := DNFunctions^.GetString(Integer(dlPlugins0));
    end;
end;
...
{&Cdecl+}
procedure CatchCommand(Command, ObjType: SmallWord; const PluginName: ShortString; Functions, Methods: Pointer; var FinalizationProc: Pointer);
const
...
begin
  ...
  if not Initialized then
    begin
      ...
      TransportVMT(_TDialog^.VMT, TypeOf(TDialog), TypeOf(TPluginManager), TDialog_VMTSize);
  ...
  if Command=0 then
    if PluginManager=nil then
      PDesktop(DNFunctions^.SomeObjects1^.Desktop)^.Insert(New(PPluginManager, Init))
    else
