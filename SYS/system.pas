...
{Cat
   16/10/2001 - ¢­ñá ­¥ª®â®àë¥ ¨§¬¥­¥­¨ï, á®£« á®¢ ­­ë¥ á ¬®¤ã«¥¬
   DebugMem ¨ ¯à¥¤­ §­ ç¥­­ë¥ ¤«ï à ¡®âë ã«®¢¨â¥«ï ãâ¥ç¥ª ¯ ¬ïâ¨

   24/10/2001 - ¯à®æ¥¤ãà  __Far16Pas ¢ windows-¢¥àá¨¨ § ¬¥­¥­  ­ 
   § £«ãèªã; íâ  ¯à®æ¥¤ãà  á¯¥æ¨ä¨ç­  ¤«ï OS/2-¢¥àá¨¨, ¯à¨ á¡®àª¥
   Win32-EXE-è­¨ª  ®­  ¢ëª¨¤ë¢ ¥âáï, â.ª. ­¨£¤¥ ­¥ ¢ë§ë¢ ¥âáï,  
   ¢®â ¯à¨ á¡®àª¥ DLL «¨­ª¥à ¡ã¤¥â àã£ âìáï ­  ¥ñ ®âáãâáâ¢¨¥,
   ¯®áª®«ìªã ®­  ®¯¨á ­  ª ª external; ¯à¨ íâ®¬ ¯à®áâ® ¢ë¡à®á¨âì
   ¯à®æ¥¤ãàã _Far16Pas, ª®â®à ï ¤¥« ¥â jmp ­  __Far16Pas ­¥«ì§ï -
   â®£¤  ®âª®¬¯¨«¨à®¢ ­­ ï ¯à®£à ¬¬  ­¥ ¡ã¤¥â à ¡®â âì
}
...
procedure _Ext2Real(Dest: Pointer);                             // Converts Extended to Real
...
procedure _Real2Ext(Src: Pointer);                              // Converts Real to Extended
...
const
  ...
  IsMultiThread:    Boolean = False;    // True if more than one thread exists
...
//HeapBlock:        Longint = 8*1024;   // 8K
  ...
  HeapAllocFlags:   Longint = {$IFDEF OS2} $53 {$ELSE} {$IFDEF LINUX} $06 {$ELSE} $1000 {$ENDIF} {$ENDIF}; // obj_Tile+pag_Commit+pag_Read+pag_Write / mem_Commit
  HeapSemaphore:    Longint = 0;        // Semaphore used to control heap/TLS code
...
type
  ...
  {$IFDEF DEBUGMEM}
    GetMemCallerNotify: procedure(Caller: Pointer); {Cat}
  {$ENDIF}
  end;
  THeapStatus = record
...
{$IFDEF DEBUGMEM}
procedure SysGetMemCallerNotify(Caller: Pointer); {Cat}
{$ENDIF}
...
{$IFDEF LINUX}
  ...
  xcpt_Integer_Overflow         = $C0080400;
  xcpt_Array_Bounds_Exceeded    = $C0080500; // ?
...
procedure __IOPort;{$FRAME-}{$USES NONE}
  asm
    jmp [offset @funktionstabelle+ecx]
...
asm
                cld
                or      ecx,-1                  // ecx := -1
                xor     eax,eax                 // Determine string length
                repne   scasb
                sub     eax,ecx                 // eax := Length of the string
                dec     eax
                dec     eax
end;
...
{$IFDEF DEBUGMEM};
    GetMemCallerNotify: SysGetMemCallerNotify {Cat}
{$ENDIF}
  );
...
procedure InitSemaphores;
var
  SemName: String[20];
begin
  Str(SysGetProcessId, SemName);
  SemName := 'VpMemMgr' + SemName + #0;

  HeapSemaphore := SemAccessMutex( @SemName[1] );
  if HeapSemaphore = -1 then
    HeapSemaphore := SemCreateMutex( @SemName[1], True, False );
end;

procedure ReleaseSemaphores;
begin
  SemCloseMutex( HeapSemaphore );
end;

procedure EnterHeap; assembler; {&Frame-} {&Uses none}
asm
...
procedure LeaveHeap; assembler; {&Frame-} {&Uses none}
asm
                cmp     byte ptr [IsMultiThread],0
                je      @@RET
...
asm
              ...
              {$IFDEF DEBUGMEM}
                push    eax
                mov     eax,dword ptr [esp+@Uses+4]
                push    eax
                Call    MemoryManager.GetMemCallerNotify
                pop     eax
              {$ENDIF}
              {/Cat}
                Call    LeaveHeap
...
asm
                ...
                mov     eax,_Old
                mov     ecx,_New
                cmp     eax,ecx             // Compate _Old and _New
                jg      @@1                 // _Old > _New
...
asm
                mov     eax,hblAlloc
                or      eax,eax             // Is HeapBlockList nil?
                jg      @@Not0
...
// Allocate initial memory for HeapBlockList
              ...
              @@Not0:
                ...
                jg      @@Insert
                shl     eax,1               // No, allocate more memory
                mov     hblAlloc,eax
...
// Perform the insert
              @@Found:
                ...
                add     esi,edx
                mov     edx,esi
                jecxz   @@NoMove            // Insert at end of list: No move
                mov     eax,ecx
                shl     eax,2
                add     esi,eax
                mov     edi,esi
...
function SysReallocMem(P: Pointer; Size: Longint): Pointer; {&USES ebx} {&FRAME-}
asm
...
{Cat}
{$IFDEF DEBUGMEM}
procedure SysGetMemCallerNotify(Caller: Pointer);
asm
end;
{$ENDIF}
{/Cat}
...
asm
                ...
                movzx   edx,al                  // edx := Length(SubStr)
...
procedure _StrIns(Src,Dest: Pointer; DestLen,Index: Longint); assembler; {&USES eax} {&FRAME+}
var
  Buffer1,Buffer2: ShortString;
asm
                cmp     Index,1         // If Index < 1 then Index := 1
                jge     @@1
                mov     Index,1
              @@1:
                lea     eax,Buffer1
                ...
                dec     eax
                push    eax             // Count
                Call    _StrCopy        // Buffer1 := Copy(Dest,1,Index-1)
                push    Src             // Dest is already on stack
                Call    _StrConcat      // + Src
                lea     eax,Buffer2
                push    eax             // Dest
                push    Dest            // Src
                push    Index           // Index
                push    255             // Count
                Call    _StrCopy        // Buffer2 := Copy(Dest,Index,255)
                Call    _StrConcat      // Buffer1 :=  Buffer1 + Buffer2
                push    Dest            // Dest
                push    DestLen         // MaxLen
                Call    _StrStore
end;
...
procedure _StrDel(S: Pointer; Index,Count: Longint); assembler; {&USES eax} {&FRAME+}
var
  Buffer1,Buffer2: ShortString;
asm
                cmp     Count,0
                ...
                dec     eax
                push    eax             // Count
                Call    _StrCopy        // Buffer1 := Copy(S,1,Index-1)
                lea     eax,Buffer2
                push    eax             // Dest
                push    S
                mov     eax,Index
                ...
                push    eax
                push    255
                Call    _StrCopy        // Buffer1 := Copy(S,Index+Count,255)
                Call    _StrConcat      // Buffer1 := Buffer1 + Buffer2
                push    S               // Dest
                push    255             // MaxLen
                Call    _StrStore
...
asm
           ...
           lock inc     [ecx-SHS].TStrRec.RefCnt
              @@1:
                ...
                jecxz   @@RET                   // Free the old contents of Dest
                mov     edx,[ecx-SHS].TStrRec.RefCnt
                dec     edx
                jl      @@RET
           lock dec     [ecx-SHS].TStrRec.RefCnt
...
asm
              ...
              @@2:
                ...
                cmp     Value.ExtRec.ER_Significand3,SIGN_Inf_Value // Is it infinity or NAN ?
                je      @@Infinity
...
procedure _ObjCtr(VmtPtr: Longint); {&USES eax,ecx} {&FRAME-}
asm
...
const
  ...
  C0_5: Single = 0.5;
  C1:   Single = 1.0;
  PI_Mul_2: Extended = 6.28318530717958648;     // 2*PI (SIN,COS period)
...
// COMMENTS: The basic limitation of FX2M1 is that it accepts parameters
// only in the range -0.5 <= X <= 0.5. So we must reduce the parameter
// to that range.

procedure _Exp; assembler; {&USES None } {&FRAME-}
var
  TempWord: Word;
asm
                fstcw   TempWord                // Save old control word
                fldcw   CWNear                  // Set rounding mode to nearest
                fldl2e                          // ST(0) := Log2(E)
                fmul    st,st(1)                // ST(0) := X * Log2(E)
                fst     st(1)                   // ST(1) := ST(0)
                frndint                         // Round towards nearest
                fsub    st(1),st                // Now -0.5 <= X <= 0.5
                fxch
                                                // ST(0) := X, ST(1)=Int part
                f2xm1                           // ST(0) := 2^ST(0) - 1
                fadd    C1                      // add bias
...
// Converts Real to Extended

procedure _Real2Ext(Src: Pointer); assembler; {&USES eax,ebx} {&FRAME-}
var
  Ext: ExtRec;
asm
                mov     ebx,Src
...
// Converts Extended to Real

procedure _Ext2Real(Dest: Pointer); {&USES eax,ebx,ecx} {&FRAME-}
asm
                sub     esp,4*3
                fstp    TByte Ptr [esp]
                pop     eax
...
asm
              ...
              @@2:                              // Read string with number
                lodsb                           // and copy it to the buffer
                cmp     al,' '
                jbe     @@3
...
asm
              ...
              @@OK:
                xchg    esi,edi
              @@3:
                lodsb                           // Copy new name to the text
...
procedure _FileSeek(FileVar: Pointer; FilePos: Longint); {&USES ALL} {&FRAME-}
asm
                mov     edi,FileVar
                Call    OpenCheck
                jne     @@RET
...
{Cat: ¤«ï „-  ¢ë¢®¤ á®®¡é¥­¨ï "Runtime Error" ­¥ ­ã¦¥­,
      â.ª. ¢ Drivers ¥áâì á¢®ï  ­ «®£¨ç­ ï ¯à®æ¥¤ãà ,  
      ¢ ¯« £¨­®¢®© ¢¥àá¨¨ ¢®§­¨ª îâ áâà ­­ë¥ ¯à®¡«¥¬ë}
...
(*
              ...
              @@1:
                mov     ecx,ExceptionNo
                jecxz   @@WriteError            // Exception ?
                mov     eax,'xE( '              // ' (Exception '
                stosd                           //  ÀÄÄÙÀÄÄÙÀÄÄÙ
                mov     eax,'tpec'
                stosd
                mov     eax,' noi'
...
type
  TPointerList = Array[1..1] of Pointer;
...
function  SysGetThreadId:longint;orgname; external;
...
// EXPECTS:     eax     = Command line parameters
//              ecx     = Environment block@
//              edx     = Module Handle
...
{$ENDIF OS2}
// Get or create heap/TLS semaphore
                Call    InitSemaphores
{$IFDEF WIN32}
                Call    InitTidTls
{$ENDIF WIN32}
