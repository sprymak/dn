   ...
   1:  WriteLn('Error: invalid format of input TDR "', Path,'"');
    ...
    begin
      ...
      for I:=0 to PC^.Count - 1 do begin
        PF:=PC^.at(i);
        S2:=UpCaseStr(MakeNormName(S1, MakeFileName(PF^.Name)));
        Desc:='';
        ...
        if (PF^.Attr and Directory) = 0 then begin
          ...
          PAVT^.AvtNewFile( S2, Desc, False, PF^.Size, PackedDate(PF), FC.StartSector, PF^.Attr);
          Inc(FilesCounter);
        end else begin
          Nm:=MakeFileName(PF^.Name);
          if (Nm <> '.') and (Nm <> '..') then begin
            if PAVT^.AvtNewFile( S2, Desc, True, 0, PackedDate(PF), 0, PF^.Attr) <> 0 then begin
              S1:=S2+'\';
              WalkTree;
              Dec(S1[0]);
   ...
   begin
      ...
      for I:=0 to PC^.Count - 1 do begin
        ...
        and (MakeFileName(PF^.Name) <> '.') then begin
          ...
          CurDirAttr:=PF^.Attr;
          S1:=UpCaseStr(MakeNormName(S1, MakeFileName(PF^.Name))) + '\';
          WalkTree;
          Dec(S1[0]);
