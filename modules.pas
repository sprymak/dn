...
function GetProcAddress(LibHandle: Integer; ProcName: PChar; var ProcAddr: Pointer): Boolean;
...
function LoadPluginModule(const ModuleName: String; var LibHandle: Integer): Boolean;
function LoadPluginModuleAndGetProcAddress(const ModuleName: String; var LibHandle: Integer; ProcNames: array of PChar; ProcAddrs: array of PPointer): Boolean;
...
function GetProcAddress(LibHandle: Handle; ProcName: PChar; var ProcAddr: Pointer): Boolean;
begin
...
begin
  ...
  if Result then
    for I := Low(ProcAddrs) to High(ProcAddrs) do
      Result := Result and GetProcAddress(LibHandle, ProcNames[I], ProcAddrs[I]^)
  else
    for I := Low(ProcAddrs) to High(ProcAddrs) do
      ProcAddrs[I]^ := nil;
