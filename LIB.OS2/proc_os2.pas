...
begin
  ...
  if Pid = GetCurPid then Result := '>' + Result
    else Result := ' ' + Result;
end;
              ...
              begin
                ...
                while Process^.RecType=1 do
                  begin
                    ...
                    if DosQueryModuleName(Process^.ModHnd, 255, @P^.Name[1])<>0 then
                      P^.Name:='?'#0;
                    P^.Name[255]:=#0;
                    for I:=1 to 255 do
                      if P^.Name[I]=#0 then
                    ...
                    if not (FilteredList and ((Process^.SessId in [0,1])
                       ...
                       Dispose(P, Done);
                    Process:=PProcessInfo(@PByteArray(Process^.Threads)^[SizeOf(TThreadInfo)*Process^.ThreadCnt]);
                  end;
              end;
           ...
           begin
             for J := I - 1 downto 0 do
               if PProcessItem(Result^.At(J))^.SessID =
                  PProcessItem(Result^.At(I))^.SessID then
                    begin Result^.AtFree(J); Dec(I); end;
               Inc(I);
           end;
end;
